...
export const twinComponents: TwinComponentRecord[] = [
  {
    ...
    riskLevel: "medium",
    status: "潮湿敏感",
    summary: "台基前缘存在渗水返碱迹象，适合在真实接入时绑定排水与雨量监测数据。",
  ...
  },
  {
    ...
    riskLevel: "high",
    status: "瓦件松动",
    summary: "双层锥体用于模拟古建屋面体量，真实模型接入后可保留 nodeId 与风险锚点映射。",
  ...
  },
  {
    ...
    riskLevel: "high",
    status: "纵向裂缝",
    summary: "当前以两根柱体表示东侧柱组，保留构件组与单点病害的关联，便于后续接入真实构件树。",
  ...
  },
  {
    ...
    riskLevel: "low",
    status: "状态稳定",
    summary: "作为对照构件保留在面板中，便于展示切换档案而不一定命中病害点位的场景。",
...
export const twinDamagePoints: TwinDamagePoint[] = [
  {
    ...
    severityScore: 0.92,
    status: "需复核",
    description: "柱身迎风面出现贯通倾向裂缝，当前以红色热点展示并联动东侧柱组档案。",
  ...
  },
  {
    ...
    severityScore: 0.87,
    status: "预警中",
    description: "屋脊转折位置检测到瓦件滑移，适合后续对接无人机摄影测量坐标。",
  ...
  },
  {
    ...
    severityScore: 0.71,
    status: "持续监测",
    description: "台基前侧出现返碱与潮湿带，适合后续与环境传感器数据做时间序列联动。",
  ...
  },
  {
    ...
    severityScore: 0.64,
    status: "待采样",
    description: "主体外檐彩画局部褪色，用于演示构件点击与病害点切换同一档案面板。",
    suggestion: "建议补充多光谱图像，并在真实模型中保留构件 UV 区域或贴图坐标。",
